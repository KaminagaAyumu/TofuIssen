...
void vert (inout appdata_full v) {
    ...
    // オブジェクトの現在の座標から揺れのオフセットを取得
    float worldOffset = unity_WorldToObject[0].w + unity_WorldToObject[2].w;
    ...
    // 2つの波を作る
    float4 wave1 = waves + (_Time.x * _WindSpeed + worldOffset) * waveSpeed * 0.5;
    float4 wave2 = waves + (_Time.x * _WindSpeed * 2.5 + worldOffset * 1.5) * waveSpeed * 0.5;


    float4 s1,c1,s2,c2;
    FastSinCos (frac(wave1), s1,c1);
    FastSinCos (frac(wave2), s2,c2);
    ...
    float waveAmount = WeightMap.r  * (v.color.a + _ShakeBending) * 5.0;
    s *= waveAmount;
