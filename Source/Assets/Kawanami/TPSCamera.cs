...
//    private Transform m_cameraTarget;
    ...
    void SetCameraFovAndDistance()
    {
        ...
        {
            m_camera.fieldOfView = Mathf.Lerp(m_camera.fieldOfView, m_lockOnFov, Time.deltaTime * m_cameraSetLerp);
        ...
        {
            m_camera.fieldOfView = Mathf.Lerp(m_camera.fieldOfView, m_normalFov, Time.deltaTime * m_cameraSetLerp);
    ...
    {
        ChangeTargetToBoss();
        m_cameraState = CameraState.intro;
    }
    ...
    {
        // ロックオン対象をタグで検索
        GameObject target = GameObject.FindGameObjectWithTag(kPlayerTag);
        //ターゲットが見つかった場合、ロックオン対象に設定
        if (target != null)
        {
            m_target = target.transform;
